...
export const DRAW_ME_OUT_SAVE_REPAIR_MIGRATION_ID = 'draw-me-out-save-v8-repair-2026-09-04'
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (migrationMatch) {
        ...
        if (!current) { send(response, 404, { code: 'SESSION_NOT_FOUND' }); return }
        const body = await readJson(request)
        if (Object.keys(body).some(key => !['migration_id','expected_version','ruleset_version'].includes(key)) || body.migration_id !== DRAW_ME_OUT_SAVE_REPAIR_MIGRATION_ID || !counter(body.expected_version) || !counter(body.ruleset_version)) throw new RequestError(400, 'INVALID_MIGRATION')
