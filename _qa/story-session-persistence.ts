...
const directory = await mkdtemp(join(tmpdir(), 'draw-me-out-session-process-'))
const databasePath = join(directory, 'session.sqlite')
const processes = new Set<ChildProcess>()
const actionableSave = () => enterStory(createInitialSave(drawMeOut), drawMeOut)
...
try {
  ...
  const contractOptions = { cartridge: drawMeOut, databasePath, actorTokens: {}, generator: { async send(): Promise<never> { throw new Error('unused') } } }
  assert.throws(() => createStorySessionLab({ ...contractOptions, rulesetVersion: 2 }), /DATABASE_CONTRACT_MISMATCH/)
  assert.throws(() => createStorySessionLab({ ...contractOptions, cartridge: { ...drawMeOut, id: 'different-game' } }), /DATABASE_CONTRACT_MISMATCH/)
