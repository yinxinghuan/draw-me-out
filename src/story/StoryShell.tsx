  ...
  return <section className={stageClass} style={stageStyle} aria-label={t(cartridge.locale, 'currentScene')}>
    <figure className={`ct-stage__media is-${status}${videoUrl ? ' has-video' : ''}${holdingPrevious ? ' is-holding-previous' : ''}`}>
      ...
      {overlayBlock && <div className={`ct-stage__caption ct-stage__caption--${overlayBlock.kind}`}>
        ...
        {captionPages.length > 1 && <button type="button" className="ct-stage__caption-page" aria-label={t(cartridge.locale, 'nextCaptionPage')} onClick={() => setCaptionPage((current) => scene <= 3 && turnPhase === 'decision' ? Math.min(current + 1, captionPages.length - 1) : (current + 1) % captionPages.length)}><span>{captionPage + 1}/{captionPages.length}</span><Icon name="arrow" /></button>}
      </div>}
...
export function StoryGameView({ cartridge, engine, player, onSelect, onLocaleChange, uiVariant }: { cartridge: StoryCartridge; engine: StoryEngineView; player: PlayerProfile; onSelect: (id: string) => void; onLocaleChange: (locale: Locale) => void; uiVariant: UiVariant }) {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      const canChoose = turnPhase === 'decision' && (engine.save.scene > 3 || decisionNarrativeReady)
...
function SessionBoundGame({ cartridge, player, bootstrap, onSelect, onLocaleChange, uiVariant }: {
  cartridge: StoryCartridge
  player: PlayerProfile
  bootstrap: NonNullable<ReturnType<typeof useStorySessionBootstrap>>
  ...
  const imageHeight = cartridge.mediaDirector?.imageTarget.height ?? 360
  const imageIdentity = useAvatarImageReference(player.imageRefUrl, player.loaded, imageWidth, imageHeight)
  const engine = useStorySessionEngine({ cartridge, ...bootstrap, imageIdentity })
  return <StoryGameView cartridge={cartridge} engine={engine} player={player} onSelect={onSelect} onLocaleChange={onLocaleChange} uiVariant={uiVariant} />
}
...
function SessionGame(props: { cartridge: StoryCartridge; onSelect: (id: string) => void; onLocaleChange: (locale: Locale) => void; uiVariant: UiVariant }) {
  const player = usePlayerProfile()
  const bootstrap = useStorySessionBootstrap('draw-me-out', props.cartridge)
