...
export function useStoryEngine(cartridge: StoryCartridge, initialMode: StoryMode, incomingChatId?: string, imageIdentity: { ready: boolean; refUrl?: string } = { ready: true }) {
  const cloud = useGameSave<PersistedStoryData>('draw-me-out')
  ...
  const act = useCallback(async (action: string, actionLocale: Locale = cartridge.locale) => {
    ...
    try {
      ...
      // legal follow-ups and one atomic state transaction. Sending them to the
      // model adds latency and lets unrelated prose contaminate a stable beat.
      // Open actions still use the selected adapter normally.
