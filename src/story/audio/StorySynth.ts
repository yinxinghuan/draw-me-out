...
export function createAmbientTexture(
  ...
  const channels = ([0, 1] as const).map(() => {
    ...
    const smoothing = material === 'harbor' ? .972 : material === 'wayfarer' ? .958 : material === 'latent' ? .95 : .94
    let fast = 0
    let slow = 0
...
export class StorySynth {
  ...
  private playSynthCue(cue: StoryAudioCue): void {
    ...
    if (cue === 'open') {
      this.woodKnock(latent ? .08 : .15)
      this.metalStrike(latent ? .045 : .075, .11, true)
      this.tone('sfx', latent ? 310 : 92, latent ? 86 : 72, latent ? .34 : .62, latent ? 'triangle' : 'sine', latent ? .035 : .055, .04)
    }
    if (cue === 'action') {
      this.paperFlick(latent ? .075 : woody ? .12 : softer ? .095 : .14)
      this.woodKnock(latent ? .042 : woody ? .075 : .065, .055)
    }
    if (cue === 'success') {
      this.woodKnock(.14)
  ...
  private startAmbient(): void {
    ...
    rainFilter.frequency.value = theme.material === 'wayfarer' ? 2850 : theme.material === 'harbor' ? 2300 : theme.material === 'latent' ? 1850 : 3250
    rainFilter.Q.value = .42
    rainGain.gain.value = theme.material === 'wayfarer' ? .34 : theme.material === 'latent' ? .2 : .27
    ...
    bodyFilter.frequency.value = theme.material === 'wayfarer' ? 310 : theme.material === 'harbor' ? 220 : theme.material === 'latent' ? 440 : 380
    bodyFilter.Q.value = .35
    bodyGain.gain.value = theme.material === 'wayfarer' ? .22 : theme.material === 'latent' ? .12 : .16
  ...
  private playMusicStep(): void {
    ...
    if (step === 0) {
      ...
      chord.forEach((interval, index) => {
        this.tone('music', frequency(theme.rootHz, interval + (index ? 12 : 0)), frequency(theme.rootHz, interval + (index ? 12 : 0)), beat * 13.5, theme.material === 'harbor' || theme.material === 'latent' ? 'triangle' : 'sine', theme.material === 'latent' ? .024 - index * .004 : .032 - index * .005)
      })
    }
    ...
    if (sparseSteps.includes(step)) {
      ...
      this.tone('music', frequency(theme.rootHz * 2, interval), frequency(theme.rootHz * 2, interval), theme.material === 'harbor' ? .28 : theme.material === 'latent' ? .2 : .48, theme.material === 'harbor' || theme.material === 'latent' ? 'triangle' : 'sine', theme.material === 'latent' ? .018 : .024)
    }
