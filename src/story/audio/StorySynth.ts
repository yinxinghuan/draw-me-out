...
export class StorySynth {
  ...
  cue(cue: StoryAudioCue): void {
    ...
    const latent = theme.material === 'latent'
    if (cue === 'open') {
      this.tone('sfx', latent ? root * 5.5 : root * 2, latent ? root * 2.1 : root * 2.5, latent ? .34 : .22, softer ? 'sine' : 'triangle', .34)
      this.tone('sfx', latent ? root * 1.5 : root * 2.5, latent ? root * 3.25 : root * 3, latent ? .18 : .28, 'sine', .22, .09)
    }
    if (cue === 'action') {
      this.noise(latent ? .038 : .055, latent ? 1680 : softer ? 950 : woody ? 720 : 1450, latent ? .13 : softer ? .15 : woody ? .12 : .22)
      this.tone('sfx', latent ? 680 : softer ? 460 : woody ? 220 : 520, latent ? 430 : softer ? 390 : woody ? 180 : 410, latent ? .065 : woody ? .09 : .075, latent ? 'square' : 'triangle', latent ? .12 : woody ? .2 : .24)
    }
    if (cue === 'success') {
  ...
  private startAmbient(): void {
    ...
    for (let index = 0; index < samples.length; index += 1) {
      ...
      smooth = theme.material === 'harbor' ? smooth * .986 + white * .014 : theme.material === 'wayfarer' ? smooth * .972 + white * .028 : theme.material === 'latent' ? smooth * .952 + white * .048 : smooth * .94 + white * .06
    ...
    noise.buffer = buffer
    noise.loop = true
    filter.type = theme.material === 'harbor' || theme.material === 'latent' ? 'bandpass' : 'lowpass'
    filter.frequency.value = theme.material === 'harbor' ? 590 : theme.material === 'wayfarer' ? 720 : theme.material === 'latent' ? 1180 : 880
    filter.Q.value = theme.material === 'harbor' ? .55 : theme.material === 'latent' ? 1.2 : .3
    gain.gain.value = theme.material === 'harbor' ? .7 : theme.material === 'wayfarer' ? .5 : theme.material === 'latent' ? .3 : .42
    ...
    drone.type = 'sine'
    drone.frequency.value = theme.material === 'harbor' ? 55 : theme.material === 'wayfarer' ? 73.42 : theme.material === 'latent' ? 49 : 60
    droneGain.gain.value = theme.material === 'harbor' ? .07 : theme.material === 'wayfarer' ? .018 : theme.material === 'latent' ? .032 : .025
  ...
  private playMusicStep(): void {
    ...
    if (step === 0) {
      ...
      chord.forEach((interval, index) => {
        this.tone('music', frequency(theme.rootHz, interval + (index ? 12 : 0)), frequency(theme.rootHz, interval + (index ? 12 : 0)), beat * 7.2, theme.material === 'harbor' || theme.material === 'latent' ? 'triangle' : 'sine', theme.material === 'latent' ? .036 - index * .006 : .055 - index * .008)
      })
    }
    ...
    if (step % pulseEvery === 2 % pulseEvery) {
      ...
      this.tone('music', frequency(theme.rootHz * 2, interval), frequency(theme.rootHz * 2, interval), theme.material === 'harbor' ? .3 : theme.material === 'latent' ? .17 : .52, theme.material === 'harbor' || theme.material === 'latent' ? 'triangle' : 'sine', theme.material === 'latent' ? .032 : .045)
    }
